...
lanspike::lanspike() : UplinkTask()
{

	status = lanspike_UNUSED;
	progress = 0;
	numticksrequired = 0;
	comp = NULL;
	systemIndex = 0;
...
void lanspike::BorderDraw ( Button *button, bool highlighted, bool clicked )
{
...
void lanspike::ProgressDraw ( Button *button, bool highlighted, bool clicked )
{
...
void lanspike::CloseClick ( Button *button )
{
...
void lanspike::BorderClick ( Button *button )
{
...
void lanspike::SetTarget ( UplinkObject *uo, char *uos, int uoi )
{
	...
	if ( status == lanspike_UNUSED || status == lanspike_FINISHED ) {
		...
		if ( uo->GetOBJECTID() == OID_LANCOMPUTER ) {
            ...
            if ( system->TYPE == LANSYSTEM_MAINSERVER ) {
			    ...
			    progress = 0;
			    numticksrequired = TICKSREQUIRED_SPIKEING;
...
void lanspike::MoveTo ( int x, int y, int time_ms )
{
	...
	UplinkSnprintf ( stitle, sizeof ( stitle ), "lanspike_title %d", pid );
	UplinkSnprintf ( sborder, sizeof ( sborder ), "lanspike_border %d", pid );
	UplinkSnprintf ( sprogress, sizeof ( sprogress ), "lanspike_progress %d", pid );
	UplinkSnprintf ( sclose, sizeof ( sclose ), "lanspike_close %d", pid );	
...
void lanspike::Initialise ()
{
}

void lanspike::Tick ( int n )
{
	...
	if ( IsInterfaceVisible () ) {
		...
		if ( status == lanspike_UNUSED ) {

			if ( comp ) {

				EclRegisterCaptionChange ( sprogress, "SPIKEING SERVER...", 0 );
				status = lanspike_SPIKEING;

			}

		}
		else if ( status == lanspike_SPIKEING ) {
			...
			if ( progress >= numticksrequired ) {
				...
				status = lanspike_FINISHED;            
            }

		}
        else if ( status == lanspike_FINISHED ) {
...
bool lanspike::IsInterfaceVisible ()
{
	
	int pid = SvbLookupPID ( this );

	char stitle [128];
	UplinkSnprintf ( stitle, sizeof ( stitle ), "lanspike_border %d", pid );
