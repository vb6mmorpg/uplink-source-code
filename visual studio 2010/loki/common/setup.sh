...
DetectLIBC()
{
	...
	if [ `uname -s` != Linux ]; then
		echo "glibc-2.1"
		return $status
	elif DetectLibc6; then
		if DetectGLIBC21; then
			echo "glibc-2.1"
			status=0
		else
			echo "glibc-2.0"
...
DetectLibc6()
{
	test -f `echo /lib/libc.so.6* | tail -n 1`
}

DetectGLIBC21()
{
	# On amd64 linux distributions, the 32bit compatibility library is
	# a directory other than /lib
	for lib in /lib/libc.so.6 \
		`/sbin/ldconfig -p | fgrep 'libc.so.6' | sed -e 's/^.*=> //'` ; do 
	  if [ -f "$lib" ]; then
		  if fgrep GLIBC_2.1 $lib 2> $NULL >> $NULL; then
			  return 0
		  fi
	  fi
	done
    ...
    then
      ...
      if [ ! -f "$setup_bin" ]; then
          setup_bin="setup.data/bin/$os/$arch/$setup"
        	# libc dependent version failed, trying again
          if [ ! -f "$setup_bin" ]; then
              failed=1
          fi
      fi
