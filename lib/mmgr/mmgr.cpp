...
static const	char		*memoryLogFile         = "mem.log"; // "memory.log";
static const	char		*memoryLeakLogFile     = "mel.log"; // "memleaks.log";
...
static	void	doCleanupLogOnFirstRun()
{
	if (cleanupLogOnFirstRun)
	{
		...
		log("--------------------------------------------------------------------------------");
		log("          %s - Logging file created on %s", memoryLogFile, asctime(localtime(&t)));
		log("--------------------------------------------------------------------------------");
...
static	void	dumpLeakReport()
{
	...
	fprintf(fp, "|                                                   Report for:  %02d/%02d/%04d %02d:%02d:%02d                                               |\r\n", tme->tm_mon + 1, tme->tm_mday, tme->tm_year + 1900, tme->tm_hour, tme->tm_min, tme->tm_sec);
	fprintf(fp, " ---------------------------------------------------------------------------------------------------------------------------------- \r\n");
	fprintf(fp, "\r\n");
	fprintf(fp, "\r\n");
	if (stats.totalAllocUnitCount)
	{
		fprintf(fp, "%d leak%s found:\r\n", stats.totalAllocUnitCount, stats.totalAllocUnitCount == 1 ? "":"s");
	}
	else
	{
		fprintf(fp, "Congratulations! No leaks found!\r\n");
...
bool	m_validateAllAllocUnits()
{
	...
	if (errors) log("[!] While validating all allocation units, %d allocation unit(s) were found to have problems", errors);
