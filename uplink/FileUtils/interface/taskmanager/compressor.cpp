	...
	if ( IsInterfaceVisible () ) {
		...
		if ( status == COMPRESSOR_OFF ) {
			...
			for ( int count = 0; count < n; ++count ) {
				...
				if ( data->compressed >= version ) { ++progress; }
				else { progress += (int) (version-data->compressed); }
				...
				if ( progress >= numticksrequired ) {
					...
					if ( data->TYPE == DATATYPE_DATA && data->compressed < version && data->size > 1 ) {
						Data *newdata = new Data(data);

						int reduce = version - data->compressed;	// Factor in prior compression
						if ( data->size <= reduce ) {
							reduce = data->size - 1;
						}
